...
use anyhow::Result;
use clap::Parser;
use std::{fs, net as stdnet, thread};
...
fn main() {
    ...
    let server_module_path = if cfg!(not(debug_assertions)) {
        format!(
            "../modules/{0}/target/wasm32-wasi/release/{0}.wasm",
            server_module_name
        )
    } else {
        format!(
            "../modules/{0}/target/wasm32-wasi/debug/{0}.wasm",
            server_module_name
        )
    };
...
fn run_gateway_module(wasm_path: &str, wasm_config_path: &str, allowed_host: Option<String>) {
    ...
    let wasm_funcs = instantiate(
        wasm_path,
        |store: &mut Store<Context<WasmgatewayfunctionsData>>, module, linker| {
            // Add wasm host functions to linker, allowing them to be used in wasm modules.
            hostobservability::add_to_linker(linker, |ctx| -> &mut Hostobservability {
...
fn run_server_module(
    ...
    // Call init of guest/wasm modules
    server_exports
        .init(server_store, wasm_config_path, PREOPENED_SOCKET_FD)
        .expect("Could not call the function.");
...
fn run_telemetry_module(wasm_path: &str, wasm_config_path: &str, allowed_host: Option<String>) {
    ...
    let wasm_funcs = instantiate(
        wasm_path,
        |store: &mut Store<Context<WasmtelemetryfunctionsData>>, module, linker| {
            // Add wasm host functions to linker, allowing them to be used in wasm modules.
            hostobservability::add_to_linker(linker, |ctx| -> &mut Hostobservability {
                ctx.runtime_data.as_mut().unwrap()
            })?;
