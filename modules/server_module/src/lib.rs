...
async fn run_server(fd: u32, data_read_buffer_size: u32, message_queue: Queue<String>) -> Result<(), Box<dyn Error>> {
    ...
    loop {
        ...
        if let Err(e) = stream_res {
            hostobservability::loginfo(MODULE_NAME, &format!("failed to accept connection; error = {}", e));
            continue;
        }
        
        let (socket, addr) = stream_res?;

        hostobservability::loginfo(MODULE_NAME, &format!("Connection received on a preopened socket address {addr:?}"));        
...
async fn process(mut socket: TcpStream, data_read_buffer_size: u32, mut message_queue: Queue<String>) -> Result<(), Box<dyn Error>> {
